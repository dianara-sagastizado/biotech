...
usData = await FileAttachment("counties-10m.json").json()
...
biotechData = await FileAttachment("biotech.csv").csv({typed: true})
...
// Create filter overlay on top of map
// Create filter overlay on top of map
const filterContainer = mapContainer.append("div")
  ...
  label.append("input")
    ...
    .on("change", function() {
      ...
      circles.transition().duration(300)
        .style("opacity", d => sel === "All" || d.Specialty === sel ? 0.8 : 0.1)
        .attr("r", d => sel === "All" || d.Specialty === sel ? 6 : 3);
    });
